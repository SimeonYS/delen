...
class DelenPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `delen`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM delen WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO delen (date, title, link, content)"
